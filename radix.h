...
static inline void rdx_init_header(void *base, uint32_t node_cap, uint32_t arena_cap, uint64_t total_size) {
    ...
    __atomic_store_n(&hdr->magic, RDX_MAGIC, __ATOMIC_RELEASE);
    ...
    if (anonymous) {
        ...
        if (!is_new) {
            if (!rdx_validate_header((RdxHeader *)base, (uint64_t)st.st_size)) {
                 ...
                 * ftruncate and rdx_init_header below leaves a full-size, all-zero
                ...
                if (((RdxHeader *)base)->magic == 0 && (uint64_t)st.st_size == total
                    && st.st_uid == geteuid()) {
                    RDX_ERR("%s: incomplete radix-tree file left by an interrupted create; remove it and retry", path);
                    munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                }
